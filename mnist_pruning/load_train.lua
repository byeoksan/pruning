...
elseif opt.model == 'cnnA' then
    ...
    model:add(nn.SpatialConvolution(1,6,7,7))
    --pool1 2*2     output: 11*11 @ 6
    model:add(nn.SpatialMaxPooling(2,2))
    model:add(nn.Reshape(11*11*6))
    --fc1 40 hidden 
    model:add(nn.Linear(11*11*6, 40))
...
elseif opt.model == 'cnnB' then
    ...
    print('Network \n'..model:__tostring())
elseif opt.model == 'load' then
    model = torch.load("model/cnn-mnist-model.t7")
end
...
-- 4. Save the model
-----------------------------------------------------------------------
filename = opt.saveDir..'cnn-mnist-model_pruning.t7'
