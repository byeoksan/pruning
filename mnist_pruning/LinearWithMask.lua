...
function LinearWithMask:clone_weight()
    self.weight_clone = self.weight:clone()
    self.bias_clone = self.bias:clone()
end

function LinearWithMask:weight_diff_square()
...
function LinearWithMask:cal_sensitivity()
...
function LinearWithMask:prune_sensitivity(ratio)
    if ratio > 1 or ratio < 0 then
        return
    end
...
function LinearWithMask:prune_sense_range(lower, upper)
    if lower > upper then
        lower, upper = upper, lower
    end
